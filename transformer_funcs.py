...
def Self_Attention(K, V, Q, use_mask=False):
	...
	weights = tf.matmul(Q, tf.transpose(K, perm=[0, 2, 1])) / (K.shape[2] ** 0.5)
	if use_mask:
		weights += atten_mask
	# 2) build new embeddings by applying the attention weights to the values matrices
	return tf.matmul(tf.nn.softmax(weights), V)
...
class Atten_Head(tf.keras.layers.Layer):
	def __init__(self, input_size, output_size, use_mask):
		...
		self.wK = self.add_weight(shape=(input_size, output_size), initializer='random_normal', trainable=True)
		self.wV = self.add_weight(shape=(input_size, output_size), initializer='random_normal', trainable=True)
		self.wQ = self.add_weight(shape=(input_size, output_size), initializer='random_normal', trainable=True)
